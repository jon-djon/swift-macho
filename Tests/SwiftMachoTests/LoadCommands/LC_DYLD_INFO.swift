...
struct LC_DYLD_INFO_Tests {
    // LC_DYLD_INFO with all sections populated
    // Header: cmd=0x22, cmdsize=0x30 (48)
    let dataWithAllSections = Data([
        0x22, 0x00, 0x00, 0x00,  // cmd = LC_DYLD_INFO (0x22)
        ...
        0x00, 0x03, 0x00, 0x00,  // exportSize = 0x300
    ])

    // LC_DYLD_INFO with no sections (all zeros)
    let dataNoSections = Data([
        ...
        0x00, 0x00, 0x00, 0x00,  // exportSize = 0
    ])

    // LC_DYLD_INFO with only some sections populated
    let dataPartialSections = Data([
        ...
        0x00, 0x01, 0x00, 0x00,  // exportSize = 0x100
    ])
    ...
    @Test
    func LC_DYLD_INFO_WithAllSections() throws {
        try dataWithAllSections.withParserSpan { span in
            let f = try SwiftMachO.LC_DYLD_INFO(parsing: &span, endianness: .little)
            #expect(f.header.id == .LC_DYLD_INFO)
            ...
            #expect(f.exportOff == 0x1500)
            #expect(f.exportSize == 0x300)
            #expect(span.count == 0)
        }
    }

    @Test
    func LC_DYLD_INFO_NoSections() throws {
        try dataNoSections.withParserSpan { span in
            ...
            #expect(f.exportOff == 0)
            #expect(f.exportSize == 0)
            #expect(span.count == 0)
        }
    }

    @Test
    func LC_DYLD_INFO_PartialSections() throws {
        try dataPartialSections.withParserSpan { span in
            ...
            #expect(f.exportOff == 0x20C0)
            #expect(f.exportSize == 0x100)
            #expect(span.count == 0)
        }
    }

    @Test
    func LC_DYLD_INFO_InvalidCommand() throws {
        _ = bad.withParserSpan { span in
            #expect(throws: ParsingError.self) {
                _ = try SwiftMachO.LC_DYLD_INFO(parsing: &span, endianness: .little)
            }
        }
    }
}
