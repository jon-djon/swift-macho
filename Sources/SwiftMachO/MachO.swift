    ...
    public init(parsing input: inout ParserSpan) throws {
        ...
        for (i, loadCommand) in cmds.enumerated() {
            do {
                ...
                case .LC_LOAD_DYLIB:
                    guard let cmd = loadCommand as? LC_LOAD_DYLIB else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_ENCRYPTION_INFO_64:
                    ...
                    loadCommands.append(
                        LoadCommandValue.LC_ENCRYPTION_INFO_64(
                            cmd, try LinkEditRaw(parsing: &span, endianness: endianness)))
                case .LC_SEGMENT:
                    guard let cmd = loadCommand as? LC_SEGMENT else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_SYMTAB:
                    ...
                    let symbols: [Symbol] = try Array(parsing: &span, count: Int(cmd.numSymbols)) {
                        ...
                        return try Symbol(
                            parsing: &symbolSpan, endianness: endianness, is64it: is64Bit)
                    }
                ...
                case .LC_UNIXTHREAD:
                    guard let cmd = loadCommand as? LC_UNIXTHREAD else {
                        throw MachOError.unknownError
                    }
                    loadCommands.append(LoadCommandValue.LC_UNIXTHREAD(cmd))
                case .LC_LOADFVMLIB:
                    guard let cmd = loadCommand as? LC_LOADFVMLIB else {
                        throw MachOError.unknownError
                    }
                    loadCommands.append(LoadCommandValue.LC_LOADFVMLIB(cmd))
                case .LC_IDFVMLIB:
                    guard let cmd = loadCommand as? LC_IDFVMLIB else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_FVMFILE:
                    guard let cmd = loadCommand as? LC_FVMFILE else {
                        throw MachOError.unknownError
                    }
                    loadCommands.append(LoadCommandValue.LC_FVMFILE(cmd))
                case .LC_PREPAGE:
                    guard let cmd = loadCommand as? LC_PREPAGE else {
                        throw MachOError.unknownError
                    }
                    loadCommands.append(LoadCommandValue.LC_PREPAGE(cmd))
                case .LC_DYSYMTAB:
                    guard let cmd = loadCommand as? LC_DYSYMTAB else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_ROUTINES:
                    guard let cmd = loadCommand as? LC_ROUTINES else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_SUB_CLIENT:
                    guard let cmd = loadCommand as? LC_SUB_CLIENT else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_SEGMENT_64:
                    guard let cmd = loadCommand as? LC_SEGMENT_64 else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_DYLD_INFO:
                    guard let cmd = loadCommand as? LC_DYLD_INFO else {
                        throw MachOError.unknownError
                    }
                ...
                case .LC_ATOM_INFO:
                    guard let cmd = loadCommand as? LC_ATOM_INFO else {
                        throw MachOError.unknownError
                    }
                    try input.seek(toRange: machORange)
                    try input.seek(toRelativeOffset: cmd.offset)
