...
Specifies the minimum version of watchOS required to run this binary, along with the SDK version it was built against. This is a legacy command -- modern binaries use `LC_BUILD_VERSION` instead, which covers all Apple platforms in a single command.
...
|------|-------------|--------|------|------|
| Command ID | Load command identifier (`LC_VERSION_MIN_WATCHOS`) | 0 | 4 | `UInt32` |
| Command Size | Total size of this load command (16 bytes) | 4 | 4 | `UInt32` |
| Version | Minimum watchOS version required (e.g. 5.0.0) | 8 | 4 | `SemanticVersion` |
...
Defined in [`Sources/SwiftMachO/LoadCommands/LC_VERSION_MIN_WATCHOS.swift`](../../Sources/SwiftMachO/LoadCommands/LC_VERSION_MIN_WATCHOS.swift).
